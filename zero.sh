...
case "$OPTION" in
    ...
    27)
        ...
        # Pindahkan file tema ARIX ke direktori web
        sudo mv "$TEMP_DIR/ArixTheme.zip" /var/www/

        # Mengekstrak file ZIP dengan opsi overwrite tanpa konfirmasi
        unzip -o /var/www/ArixTheme.zip -d /var/www/

        # Hapus folder repo dan file ZIP agar bersih
        rm -r installer-premium
        rm /var/www/ArixTheme.zip

        # Menjalankan setup dependensi Node.js dan Yarn
        sudo mkdir -p /etc/apt/keyrings

        # Menambahkan GPG key dan repo NodeSource
        ...
        # Update dan install Node.js, npm, dan Yarn
        sudo apt update
        sudo apt install -y nodejs
        apt install -y npm
        npm i -g yarn
        ...
        # Jalankan build front-end
        yarn
        yarn build:production

        # Jalankan migrasi dan bersihkan cache Laravel
        php artisan migrate --force
        php artisan view:clear
        php artisan cache:clear

        # Pesan sukses
        animate_text "Tema ARIX berhasil diinstal."
        ;;
    4)
        # Masukkan token GitHub langsung di sini
        GITHUB_TOKEN="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"
